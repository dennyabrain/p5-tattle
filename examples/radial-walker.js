...
new p5((sketch) => {
    ...
    const grid = pipe(
        isometricGrid({ width: WIDTH, height: HEIGHT, cellSize: 80 }),
    ...
    sketch.draw = () => {
        ...
        for (const region of radialWalker(grid, { origin: [WIDTH / 2, HEIGHT / 2] })) {
            sketch.push()
            sketch.translate(region.center[0], region.center[1])
            sketch.rotate(135)
            sketch.translate(-region.center[0], -region.center[1])

            const colorIx = Math.floor(sketch.map(region.distanceToOrigin, 0, 420, 0, colors.length - 1))
